...
import util.Convert;
import util.HttpUtil.HttpContentBean;
import util.JacksonUtil;
import util.Pool;
import util.compress.GZipUtils;
...
public class RequestHandler implements Pool.Poolable {
    ...
    public Map<String, List<String>> getQueryAttrs() {
        if (uriAttributes == null) {
            uriAttributes = getQueryAttrs(this.getRequest().uri());
        }
    ...
    protected FullHttpResponse buildResponse(final HttpResponseStatus status, final byte[] data) {
        ...
        util.HttpUtil.HttpContentBean bean = beforeBuildResponse(data);
    ...
    protected util.HttpUtil.HttpContentBean beforeBuildResponse(final byte[] data) {
        return null;
    }
    ...
    protected void afterBuildResponse(final HttpResponseStatus status,
        ...
        response.headers().set(HttpHeaderNames.CONTENT_TYPE, util.HttpUtil.TYPE_PLAIN);
